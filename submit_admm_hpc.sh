...
OUT_DIR="data/ml_dataset/${SLURM_ARRAY_JOB_ID}"
...
if [[ -z "$CASE_NAME" ]]; then
    ...
    echo "Usage: sbatch submit_admm.sh --case <case_name>"
    exit 1
fi
...
# Pass the case name, task ID, and the dynamically created output directory
python generate_admm_dataset.py \
    --case "$CASE_NAME" \

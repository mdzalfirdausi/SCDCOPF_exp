...
def create_zonal_data(case, zone1_buses, zone2_buses):
    ...
    # Extract Global Generator and Line Contingencies
...
class Zone_ADMM_GNN(nn.Module):
    def __init__(self, num_boundaries, num_global_kg, num_global_ke):
        super(Zone_ADMM_GNN, self).__init__()
        
        self.num_kg_base = num_global_kg + 1 
        self.num_boundaries = num_boundaries
        self.num_global_ke = num_global_ke 
        ...
        self.ek_head = nn.Sequential(
            nn.Linear(128 + (num_global_ke * 2), 128),
            nn.LeakyReLU(0.1),
            nn.Linear(128, num_global_ke),
            nn.Sigmoid() 
        )

    def forward(self, data, Va_target, u_va, zk_target, u_zk, ek_target, u_ek):
        ...
        Va_local = (torch.tanh(self.va_head(h_global_va)) * math.pi).view(num_graphs, self.num_kg_base, self.num_boundaries)
        
        h_global_zk = torch.cat([h_global, zk_target, u_zk / 10000.0], dim=1)
        zk_local = self.zk_head(h_global_zk)
        
        h_global_ek = torch.cat([h_global, ek_target, u_ek / 10000.0], dim=1)
        ek_local = self.ek_head(h_global_ek)
...
# ==========================================
def compute_zonal_erdos_loss(Pg_base, Va_local, zk_local, ek_local, Pd_batch, 
    ...
    # Generator Contingency Consensus
    zk_diff = zk_local - zk_target
    admm_zk_linear = torch.sum(u_zk * zk_diff, dim=1)
    admm_zk_quad = torch.sum((rho_ADMM / 2.0) * (zk_diff ** 2), dim=1)
    
    # Line Contingency Consensus
    ek_diff = ek_local - ek_target
    admm_ek_linear = torch.sum(u_ek * ek_diff, dim=1)
    admm_ek_quad = torch.sum((rho_ADMM / 2.0) * (ek_diff ** 2), dim=1)
    
    erdos_zk_penalty = torch.sum(zk_local * (1.0 - zk_local), dim=1)
    security_zk_penalty = torch.sum(1.0 - zk_local, dim=1) 
    
    erdos_ek_penalty = torch.sum(ek_local * (1.0 - ek_local), dim=1)
    security_ek_penalty = torch.sum(1.0 - ek_local, dim=1) 
    ...
    total_loss = (
        ...
        + (beta_erdos * torch.mean(erdos_zk_penalty + erdos_ek_penalty)) 
        + (beta_sec * torch.mean(security_zk_penalty + security_ek_penalty))
    )
    return total_loss
...
# 4. TRAINING ENGINE
# ==========================================
def train_agent_gnn(zone_name, case_name, z_data, load_data_np, num_boundaries, num_global_kg, num_global_ke, baseMVA, rho_ADMM, epochs=150, batch_size=128):
    ...
    net = Zone_ADMM_GNN(num_boundaries, num_global_kg, num_global_ke).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            ...
            # Simulate ADMM messages randomly for unsupervised training
            ...
            Pg_base, Va_local, zk_local, ek_local = net(batch, Va_target, u_va, zk_target, u_zk, ek_target, u_ek)
            Pd_batch = batch.Pd.view(current_batch_size, -1)
            
            loss = compute_zonal_erdos_loss(Pg_base, Va_local, zk_local, ek_local, Pd_batch, 
                                            Va_target, u_va, zk_target, u_zk, ek_target, u_ek,
...
if __name__ == "__main__":
    ...
    train_agent_gnn('zone1', case_name, zonal_data['zone1'], Pd_z1_np, num_boundaries, num_global_kg, num_global_ke, baseMVA, rho_ADMM, epochs=epochs)
    train_agent_gnn('zone2', case_name, zonal_data['zone2'], Pd_z2_np, num_boundaries, num_global_kg, num_global_ke, baseMVA, rho_ADMM, epochs=epochs)

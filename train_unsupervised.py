...
def create_zonal_data(case, zone1_buses, zone2_buses):
    zonal_data = {'zone1': {}, 'zone2': {}, 'tie_lines': [], 'boundary_buses': [], 'global_Kg': [], 'global_Ke': []}
    zonal_data['global_Kg'] = [int(x) for x in case['gen']['gen_ID'].tolist()]
    zonal_data['global_Ke'] = [int(x) for x in case['branch']['line_ID'].tolist()]
    ...
    for z in ['zone1', 'zone2']:
        ...
        zonal_data[z]['global_Ke'] = zonal_data['global_Ke']
...
class Zone_ADMM_Net(nn.Module):
    def __init__(self, num_local_buses, num_local_gens, num_boundaries, num_global_kg, num_global_ke, Pmax, Pmin):
        ...
        self.out_ek_dim = num_global_ke
        
        # Inputs: Load + Va_target + u_va + zk_target + u_zk + ek_target + u_ek
        input_dim = num_local_buses + (self.num_kg_base * num_boundaries) * 2 + (num_global_kg * 2) + (num_global_ke * 2)
        
        self.out_pg_dim = num_local_gens
        self.out_va_dim = self.num_kg_base * num_boundaries
        output_dim = self.out_pg_dim + self.out_va_dim + self.out_zk_dim + self.out_ek_dim
    ...
    def forward(self, Pd, Va_target, u_va, zk_target, u_zk, ek_target, u_ek):
        batch_size = Pd.shape[0]
        
        u_va_norm = u_va.view(batch_size, -1) / 10000.0
        u_zk_norm = u_zk / 10000.0
        u_ek_norm = u_ek / 10000.0
        
        x = torch.cat([Pd, Va_target.view(batch_size, -1), u_va_norm, zk_target, u_zk_norm, ek_target, u_ek_norm], dim=1)
        raw_out = self.net(x)
        
        raw_Pg = raw_out[:, :self.out_pg_dim]
        raw_Va_flat = raw_out[:, self.out_pg_dim : self.out_pg_dim + self.out_va_dim]
        raw_zk = raw_out[:, self.out_pg_dim + self.out_va_dim : self.out_pg_dim + self.out_va_dim + self.out_zk_dim]
        raw_ek = raw_out[:, self.out_pg_dim + self.out_va_dim + self.out_zk_dim :]
        ...
        ek_local = torch.sigmoid(raw_ek)                       
        
        return Pg_base, Va_local, zk_local, ek_local

# ==========================================
# 3. UNSUPERVISED ADMM LOSS FUNCTION
# ==========================================
def compute_zonal_loss(Pg_base, Va_local, zk_local, ek_local, Pd, Va_target, u_va, zk_target, u_zk, ek_target, u_ek,
    ...
    ek_diff = ek_local - ek_target
    admm_ek_linear = torch.sum(u_ek * ek_diff, dim=1)
    admm_ek_quad = torch.sum((rho_ADMM / 2.0) * (ek_diff ** 2), dim=1)
    
    lambda_admm = 100.0 
    
    admm_loss = torch.mean(admm_va_linear + admm_va_quad + admm_zk_linear + admm_zk_quad + admm_ek_linear + admm_ek_quad)
...
# 4. TRAINING ENGINE
# ==========================================
def train_agent(zone_name, z_data, load_data_np, num_boundaries, num_global_kg, num_global_ke, baseMVA, rho_ADMM, epochs=150, batch_size=128):
    ...
    net = Zone_ADMM_Net(num_local_buses, num_local_gens, num_boundaries, num_global_kg, num_global_ke, Pmax, Pmin).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch_Pd in dataloader:
            ...
            ek_target = torch.rand(current_batch_size, num_global_ke).to(device)
            u_ek = torch.randn(current_batch_size, num_global_ke).to(device) * rho_ADMM
            
            Pg_base, Va_local, zk_local, ek_local = net(batch_Pd, Va_target, u_va, zk_target, u_zk, ek_target, u_ek)
            
            loss = compute_zonal_loss(Pg_base, Va_local, zk_local, ek_local, batch_Pd, Va_target, u_va, zk_target, u_zk, ek_target, u_ek,
...
if __name__ == "__main__":
    ...
    # Extract only the Pd columns for the MLP
    bus_list = sorted(case['bus']['bus_i'].tolist())
    Pd_np = np.array([[row[f"Bus_{b}_Pd"] for b in bus_list] for _, row in load_data.iterrows()]) / baseMVA
    
    Pd_z1_np = Pd_np[:, :num_buses_z1]
    Pd_z2_np = Pd_np[:, num_buses_z1:]
    
    epochs = 1500

    train_agent('zone1', zonal_data['zone1'], Pd_z1_np, num_boundaries, num_global_kg, num_global_ke, baseMVA, rho_ADMM, epochs=epochs)
    train_agent('zone2', zonal_data['zone2'], Pd_z2_np, num_boundaries, num_global_kg, num_global_ke, baseMVA, rho_ADMM, epochs=epochs)
